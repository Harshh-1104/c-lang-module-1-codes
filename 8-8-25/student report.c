...

{
    ...
    int num,sub[100][3],total[100],pass=0,fail=0,topper=0,loser=0,high=0,low=0;
    char name[100][30];
    float avg[100];
    ...
    printf("\n%s         %d          %f           %s",name[i],total[i],avg[i], (avg[i] >= 40) ? "pass" : "fail");
    ...
    printf("\nTopper: %s  (%f)", name[topper], avg[topper]);
    printf("\nLoser:  %s  (%f)", name[loser], avg[loser]);
    printf("\nPassed: %d", pass);
    printf("\nFailed: %d", fail);
